...
//
//namespace tcp {
//Define_Module(TCPFit);
Register_Class (inet::tcp::TCPFit);
std::vector<inet::tcp::TCPFit>ssthreshVector;
std::vector<inet::tcp::TCPFit>cwndVector;
inet::tcp::TCPFitStateVariables::TCPFitStateVariables() {
    // init
    update_epoch = 0;
    ssthresh = 0xffffffff;
    snd_cwnd = 2;
...
std::string inet::tcp::TCPFitStateVariables::info() const {
    std::stringstream out;
    out <<TcpBaseAlgStateVariables::info();
    EV<<"State Variables Init";
    out << " ssthresh=" << ssthresh;
    return out.str();
}

std::string inet::tcp::TCPFitStateVariables::detailedInfo() const {
    std::stringstream out;
    out << TcpBaseAlgStateVariables::detailedInfo();
//    out << "ssthresh = " << ssthresh << "\n";
//    out << "w_RTTmin = " << w_RTTmin << "\n";
   EV << "ssthresh = " << ssthresh << "\n";
   EV << "w_RTTmin = " << w_RTTmin << "\n";
    return out.str();
}

/* Ctor */
inet::tcp::TCPFit::TCPFit() :
...
void inet::tcp::TCPFit::recalculateSlowStartThreshold() {
    ...
    //a vector required to record the current ssthresh
    if (ssthreshVector) {
        ssthreshVector->record(state->ssthresh);
    }
}

void inet::tcp::TCPFit::processRexmitTimer(TcpEventCode& event) {
...
//            state->snd_cwnd = 2;
//            tcpFitReset();
//a vector required to record the snd_cwnd
    if (cwndVector)
...
void inet::tcp::TCPFit::receivedDataAck(uint32 firstSeqAcked) {
...
void inet::tcp::TCPFit::receivedDuplicateAck() { // everything like TCP Reno
...
void inet::tcp::TCPFit::dataSent(uint32 fromseq) {
...
void inet::tcp::TCPFit::segmentRetransmitted(uint32 fromseq, uint32 toseq) {
...
void inet::tcp::TCPFit::tcpFitUpdateN() {
    currentTime = simTime();
    if (state->beta == 0) {
...
void inet::tcp::TCPFit::tcpFitReset() {
    state->w_RTTmin = 0;
    state->RTT_cnt = 0;
    state->ACK_cnt = 0;
    currentTime = simTime();
